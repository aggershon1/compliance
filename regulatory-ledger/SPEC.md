...
> **Retired in v0.8.0.** The folder-upload entry path has been removed — source code in a browser tool doesn't survive most security reviews. Entries created before the retirement remain viewable and still count toward their scores; the analysis engine survives in `js/codeaudit.js` (no longer loaded) for a future GitHub App or CLI path. See [`ROADMAP.md`](./ROADMAP.md). This section is kept as the design record of how the track worked.
