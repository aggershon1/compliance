...
- **The app is still a static file, but there is now an optional backend.** `server/` holds a Node crawl service that exists for two reasons: a browser cannot read another origin's pages (same-origin policy), and a static file cannot hold an API key. The app degrades cleanly when it isn't running, when the agents aren't installed, and when the API is unreachable — and it always says *which* reviewer or discovery method actually ran.
  ...
  - **Keep the split intact — the server retrieves, the client judges.** Website compliance logic must not migrate into `server/`; a service that only reports what it fetched cannot invent a finding about a real company. The navigator agent doesn't break this: it decides *which pages to open*, never what they mean. The one judgment that does live server-side is `agent/attest.js`, and it judges only the user's own written account of a behind-login flow — it has no access to the product and observes nothing.
